...
class Trainer:
    ...
    def __init__(
        ...
        self.lr_scheduler = ReduceLROnPlateau(self.optimizer, mode='min', factor=lr_factor, patience=lr_patience)
        self.best_loss = None
        self.save_model_path = Path(save_model_path).resolve()
    ...
    def checkpoint(
        ...
        и процесса тренировки в чекпоинт."""
    ...
    def train_model(
        ...
        print(f'Epoch [{(epoch + 1)} / {self.epochs}]: Train loss = {mean_loss:.4f}; Train accuracy = {train_acc:.4f}')
        return {'train_loss': mean_loss, 'train_acc': train_acc}
    ...
    def eval_model(
        ...
        lr_value = self.lr_scheduler._last_lr[0]
        print(f'Epoch [{(epoch + 1)} / {self.epochs}]: Validation loss = {mean_loss:.4f}; Validation accuracy = {valid_acc:.4f}; Learning rate = {lr_value:.4f}')
        print('-' * 100)
        if mean_loss < self.best_loss:
            self.best_loss = mean_loss
            best_chkpt = self.checkpoint(valid_loss=mean_loss, valid_acc=valid_acc, save_epoch=epoch, **train_metrics)
            torch.save(best_chkpt, self.save_model_path.joinpath('mnist_classifier.pt'))
            print(f'Сохранена модель с наилучшим значением функции потерь на валидации - {mean_loss:.4f}\n{"-" * 100}\n')
    ...
    def run(self, dataset: Dataset, batch_size: int) -> None:
        ...
        for epoch in range(self.epochs):
            train_metrics = self.train_model(epoch, train_loader, len(train_data))
            self.eval_model(epoch, val_loader, len(val_data), train_metrics=train_metrics)
