...
# В данном примере skip connection содержит свёрточный слой. Данный слой называется downsample.
...
class BasicBlock(nn.Module):
    def __init__(self, inp_channels, out_channels):
        ...
        self.main_conv_layer = nn.Sequential(
            nn.Conv2d(inp_channels, 10, (3, 3), padding=1, stride=2, bias=False),
            nn.BatchNorm2d(10),
            nn.ReLU(),
            nn.Conv2d(10, out_channels, (3, 3), padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
        )
        self.branch_conv_layer = nn.Sequential(
    ...
    def forward(self, batch):
        outputs = self.main_conv_layer(batch) + self.branch_conv_layer(batch)
